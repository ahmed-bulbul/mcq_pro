package com.bulbul.examportal.utils.payments;
...
public class ParameterBuilder {
    public static String getParamsString(Map<String, String> params, boolean urlEncode) throws UnsupportedEncodingException {
        ...
        for (Map.Entry<String, String> entry : params.entrySet()) {
            if (urlEncode && entry.getKey() != null && entry.getValue() != null)
                result.append(URLEncoder.encode(entry.getKey(), "UTF-8"));
            else
                result.append(entry.getKey());

            result.append("=");
            if (urlEncode && entry.getValue() != null)
    ...
    public static Map<String, String> constructRequestParameters(PaymentDetailsDTO paymentDetailsDTO) {
        // CREATING LIST OF POST DATA
        String baseUrl = "http://localhost:8081/payment/";//Request.Url.Scheme + "://" + Request.Url.Authority + Request.ApplicationPath.TrimEnd('/') + "/";
        Map<String, String> postData = new HashMap<String, String>();
        postData.put("total_amount", paymentDetailsDTO.getTotal_amount());
        ...
        postData.put("cus_name", paymentDetailsDTO.getCus_name());
